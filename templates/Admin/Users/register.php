  ...
  <div class="container container-tight py-4">
    ...
    <div class="card card-md shadow">
      <div class="card-body">
        ...
        <?= $this->Form->create($user, ['autocomplete' => 'off', 'novalidate' => true]) ?>
          <div class="mb-3">
            <label class="form-label"><?= __('Name') ?></label>
            <input type="text" name="name" class="form-control" placeholder="<?= __('Enter name') ?>" value="<?= h($user->name ?? '') ?>" required autofocus>
          </div>
          <div class="mb-3">
            <label class="form-label"><?= __('Email address') ?></label>
            <input type="email" name="email" class="form-control" placeholder="<?= __('Enter email') ?>" value="<?= h($user->email ?? '') ?>" required>
          </div>
          <div class="mb-3">
            <label class="form-label"><?= __('Password') ?></label>
            <div class="input-group input-group-flat">
              <input type="password" name="password" id="register-password" class="form-control" placeholder="<?= __('Password') ?>" autocomplete="new-password" required>
              <span class="input-group-text">
                <a href="#" id="toggle-password-btn" class="link-secondary" title="<?= __('Show password') ?>" data-bs-toggle="tooltip">
          ...
          <div class="mb-3">
            <label class="form-check">
              ...
              <span class="form-check-label">
                <?= __('Agree the') ?> 
                <?= $this->Html->link(__('terms and policy'), ['prefix' => 'Admin', 'controller' => 'Pages', 'action' => 'display', 'terms'], ['tabindex' => '-1', 'target' => '_blank']) ?>.
              </span>
            </label>
          </div>
      ...
      <div class="card-body">
        <div class="row">
          <div class="col">
            <?= $this->Html->link(
                '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon text-facebook icon-2"><path d="M7 10v4h3v7h4v-7h3l1 -4h-4v-2a1 1 0 0 1 1 -1h3v-4h-3a5 5 0 0 0 -5 5v2h-3"></path></svg>' . __('Sign up with Facebook'),
                ['prefix' => 'Admin', 'controller' => 'Users', 'action' => 'socialLogin', 'facebook'],
                ['escape' => false, 'class' => 'btn w-100']
            ) ?>
          </div>
          <div class="col">
            <?= $this->Html->link(
                '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon text-google icon-2"><path d="M20.945 11a9 9 0 1 1 -3.284 -5.997l-2.655 2.392a5.5 5.5 0 1 0 2.119 6.605h-4.125v-3h7.945z"></path></svg>' . __('Sign up with Google'),
                ['prefix' => 'Admin', 'controller' => 'Users', 'action' => 'socialLogin', 'google'],
                ['escape' => false, 'class' => 'btn w-100']
            ) ?>
          </div>
        </div>
    ...
    <div class="text-center text-secondary mt-3">
      <?= __('Already have account?') ?> <?= $this->Html->link(__('Sign in'), ['prefix' => 'Admin', 'controller' => 'Users', 'action' => 'login'], ['tabindex' => '-1']) ?>
    </div>
  </div>
</div>
