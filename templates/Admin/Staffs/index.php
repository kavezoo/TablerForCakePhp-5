...
$lastViewedId = $lastViewedId ?? $this->getRequest()->getSession()->read('LastViewed.staff_id');
$scrollToId = $scrollToId ?? $this->getRequest()->getSession()->read('ScrollTo.staff_id') ?? $lastViewedId;
...
<?php endif; ?>
            ...
            <tbody>
                ...
                <tr id="row-<?= (int)$staff->id ?>" class="<?= $isLastViewed ? 'last-viewed' : '' ?>">
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($staff->id) ?></td>
<?php endif; ?>
                    <td class="string user_id">
						<?= $staff->hasValue('user') ? $this->Html->link(
							h($staff->user->username) . '<span class="icon-link-subtle ms-1">' . $this->Icon->outline('link') . '</span>',
							...
							[
								...
								'title' => '<b>' . h($staff->user->username) . '</b><br>' . __('adatlap megtekintése'),
							]
						) : '' ?>
