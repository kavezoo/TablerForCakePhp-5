 ...
 * @var iterable<\App\Model\Entity\> $cities
...
$lastViewedId = $lastViewedId ?? $session->read('LastViewed.' . $prefix . '.city_id');
$scrollToId = $scrollToId ?? $session->read('ScrollTo.' . $prefix . '.city_id') ?? $lastViewedId;
...
<div class="card">
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új city gomb -->
            <div class="col-auto">
                <?= $this->Html->link($this->Icon->outline('plus') . '<span class="d-none d-sm-inline ms-1">' . __('Add new') . ' ' . __('city') . '</span>', ['controller' => 'Cities', 'action' => 'add'], ['escape' => false, 'class' => 'btn btn-outline-secondary btn-header-new']) ?>
            </div>
            ...
            <div class="col-12 col-md-auto ms-md-auto">
                <?= $this->KvForm->search('search', ['controller' => 'Cities', 'action' => 'index', '?' => ['clear' => 'search']]) ?>
            </div>
...
<?php endif; ?>
					<th class="string shortname"><?= $this->Paginator->sort('shortname') ?></th>					
					<th class="string name"><?= $this->Paginator->sort('name') ?></th>					
					<th class="string zip"><?= $this->Paginator->sort('zip') ?></th>					
					<th class="string lat"><?= $this->Paginator->sort('lat') ?></th>					
					<th class="string lng"><?= $this->Paginator->sort('lng') ?></th>					
...
<?php endif; ?>
            ...
            <tbody>
                ...
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $city->id); ?>
                <tr
                    id="row-<?= (int)$city->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $city->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($city->id) ?></td>
<?php endif; ?>
                    <td class="string shortname"><?= $highlight($city->shortname) ?></td>
                    <td class="string name"><?= $highlight($city->name) ?></td>
                    <td class="string zip"><?= $highlight($city->zip) ?></td>
                    <td class="string lat"><?= $highlight($city->lat) ?></td>
                    <td class="string lng"><?= $highlight($city->lng) ?></td>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <td class="integer club_count text-end"><?= $this->Number->format($city->club_count) ?></td>
<?php endif; ?>
<?php if (isset($showCounterFields) && $showCounterFields): ?>
                    <td class="integer user_count text-end"><?= $this->Number->format($city->user_count) ?></td>
<?php endif; ?>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $city->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($city->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($city->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($city->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $city->id]) ?>
