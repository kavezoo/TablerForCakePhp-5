...
<?php endif; ?>
            ...
            <tbody>
                ...
                <tr id="row-<?= (int)$socialAccount->id ?>" class="<?= $isLastViewed ? 'last-viewed' : '' ?>">
<?php if (isset($showId) && $showId): ?>
                    <td class="uuid id"><?= h($socialAccount->id) ?></td>
<?php endif; ?>
                    <td class="string user_id">
						<?= $socialAccount->hasValue('user') ? $this->Html->link(
							h($socialAccount->user->username) . '<span class="icon-link-subtle ms-1">' . $this->Icon->outline('link') . '</span>',
							...
							[
								...
								'title' => '<b>' . h($socialAccount->user->username) . '</b><br>' . __('adatlap megtekintése'),
							]
						) : '' ?>
