 ...
 * @var iterable<\App\Model\Entity\> $subclubs
...
$lastViewedId = $lastViewedId ?? $this->getRequest()->getSession()->read('LastViewed.subclub_id');
$scrollToId = $scrollToId ?? $this->getRequest()->getSession()->read('ScrollTo.subclub_id') ?? $lastViewedId;
...
<div class="card">
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új subclub gomb -->
            <div class="col-auto">
                <?= $this->Html->link($this->Icon->outline('plus') . '<span class="d-none d-sm-inline ms-1">' . __('Add new') . ' ' . __('subclub') . '</span>', ['controller' => 'Subclubs', 'action' => 'add'], ['escape' => false, 'class' => 'btn btn-outline-secondary btn-header-new']) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Subclubs') ?></h3>
                <p class="text-secondary m-0"><?= __('Edit line double click on the line') ?></p>
            </div>

            <!-- Kereső űrlap -->
            <div class="col-12 col-md-auto ms-md-auto">
                <?= $this->KvForm->search('search', ['controller' => 'Subclubs', 'action' => 'index', '?' => ['clear' => 'search']]) ?>
            </div>
...
<?php endif; ?>
            ...
            <tbody>
                <?php if (!empty($subclubs) && count($subclubs) > 0): ?>
                <?php foreach ($subclubs as $subclub): ?>
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $subclub->id); ?>
                <tr
                    id="row-<?= (int)$subclub->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $subclub->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($subclub->id) ?></td>
<?php endif; ?>
                    <td class="string club_id">
						<?= $subclub->hasValue('club') ? $this->Html->link(
							h($subclub->club->name) . '<span class="icon-link-subtle ms-1">' . $this->Icon->outline('link') . '</span>',
							['controller' => 'Clubs', 'action' => 'view', $subclub->club->id],
							[
								...
								'title' => '<b>' . h($subclub->club->name) . '</b><br>' . __('adatlap megtekintése'),
							]
						) : '' ?>
						
					</td>
                    <td class="string competition_id">
						<?= $subclub->hasValue('competition') ? $this->Html->link(
							h($subclub->competition->title) . '<span class="icon-link-subtle ms-1">' . $this->Icon->outline('link') . '</span>',
							['controller' => 'Competitions', 'action' => 'view', $subclub->competition->id],
							[
								'class' => 'text-reset text-decoration-none fw-bold',
								'escape' => false,
								'data-bs-toggle' => 'tooltip',
								'data-bs-html' => 'true',
								'data-bs-placement' => 'top',
								'title' => '<b>' . h($subclub->competition->title) . '</b><br>' . __('adatlap megtekintése'),
							]
						) : '' ?>
						
					</td>
                    <td class="string name"><?= $highlight($subclub->name) ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $subclub->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($subclub->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($subclub->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($subclub->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $subclub->id]) ?>
