 ...
 * @var iterable<\App\Model\Entity\> $emailTemplates
...
$lastViewedId = $lastViewedId ?? $this->getRequest()->getSession()->read('LastViewed.emailTemplate_id');
$scrollToId = $scrollToId ?? $this->getRequest()->getSession()->read('ScrollTo.emailTemplate_id') ?? $lastViewedId;
...
<div class="card">
    <div class="card-header pe-3">
        <div class="row w-full align-items-center gy-2 gy-md-0">
            ...
            <!-- Új emailTemplate gomb -->
            <div class="col-auto">
                <?= $this->Html->link($this->Icon->outline('plus') . '<span class="d-none d-sm-inline ms-1">' . __('Add new') . ' ' . __('emailTemplate') . '</span>', ['controller' => 'Emailtemplates', 'action' => 'add'], ['escape' => false, 'class' => 'btn btn-outline-secondary btn-header-new']) ?>
            </div>

            <!-- Cím és infó blokk -->
            <div class="col">
                <h3 class="card-title mb-0"><?= __('Emailtemplates') ?></h3>
                <p class="text-secondary m-0"><?= __('Double-click on the line') . ' ' . __('edit') ?></p>
            </div>

            <!-- Kereső űrlap -->
            <div class="col-12 col-md-auto ms-md-auto">
                <?= $this->KvForm->search('search', ['controller' => 'Emailtemplates', 'action' => 'index', '?' => ['clear' => 'search']]) ?>
            </div>
...
<?php endif; ?>
					...
					<th class="string subject"><?= $this->Paginator->sort('subject') ?></th>					
...
<?php endif; ?>
            ...
            <tbody>
                ...
                <?php $isLastViewed = (!empty($lastViewedId) && $lastViewedId == $emailTemplate->id); ?>
                <tr
                    id="row-<?= (int)$emailTemplate->id ?>"
                    class="<?= $isLastViewed ? 'last-viewed' : '' ?>"
                    data-edit-url="<?= $this->Url->build(['action' => 'edit', $emailTemplate->id]) ?>"
                >
<?php if (isset($showId) && $showId): ?>
                    <td class="integer id"><?= h($emailTemplate->id) ?></td>
<?php endif; ?>
                    <td class="string name"><?= $highlight($emailTemplate->name) ?></td>
                    <td class="string slug"><?= $highlight($emailTemplate->slug) ?></td>
                    <td class="string subject"><?= $highlight($emailTemplate->subject) ?></td>
                    <td class="boolean enabled"><?= $emailTemplate->enabled ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php if (isset($showVisible) && $showVisible): ?>
                    <td class="boolean visible"><?= $emailTemplate->visible ? '<span class="badge bg-green-lt">' . __('Igen') . '</span>' : '<span class="badge bg-secondary-lt">' . __('Nem') . '</span>' ?></td>
<?php endif; ?>
<?php if (isset($showPos) && $showPos): ?>
                    <td class="integer pos"><?= h($emailTemplate->pos) ?></td>
<?php endif; ?>
<?php if ((isset($showCreated) && $showCreated) || (isset($showModified) && $showModified)): ?>
                    <td class="datetime text-nowrap">
<?php if (isset($showCreated) && $showCreated): ?>
                        <small class="d-block text-muted"><?= h($emailTemplate->created?->format('Y-m-d H:i')) ?></small>
<?php endif; ?>
<?php if (isset($showModified) && $showModified): ?>
                        <span><?= h($emailTemplate->modified?->format('Y-m-d H:i')) ?></span>
<?php endif; ?>
                    </td>
<?php endif; ?>
                    <td class="actions">
                        <div class="btn-list flex-nowrap align-items-center">
                            <?= $this->KvForm->actionView(['action' => 'view', $emailTemplate->id]) ?>
